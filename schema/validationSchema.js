...
export const jobApplySchema = yup.object().shape({
  ...
  currentCtc: yup
    ...
    .matches(REGEX.ctc, "Enter a valid CTC (₹1 to ₹10 crore)"),
  expectedCtc: yup
    .string()
    .required("Expected CTC is required")
    .matches(REGEX.ctc, "Enter a valid Expected CTC (₹1 to ₹10 crore)"),
  experience: yup
    .string()
...
export const channelPartnerSchema = yup.object().shape({
  ...
  gstin: yup
    ...
    .matches(
      REGEX.gstin,
      "GSTIN must be a valid 15-character format (e.g., 22AAAAA0000A1Z5)",
    ),
  agency_name: yup.string().when("agentType", {
    is: "Agency Registration",
    then: (schema) =>
      schema
        .required("Agency name is required")
        .matches(
          REGEX.startingSpaceNotAllowed,
  ...
  heardAboutUs: yup.string().when("agentType", {
    is: "Individual Registration",
    then: (schema) => schema.required("How did you hear about us is required"),
    otherwise: (schema) => schema.optional(),
  }),
  referFullname: yup.string().when("agentType", {
    is: "Individual Registration",
    then: (schema) =>
      schema
        .required("Name is required")
  ...
  referPhone: yup.string().when("agentType", {
    is: "Individual Registration",
    then: (schema) =>
      schema
        .required("Mobile number is required")
  ...
  referEmail: yup.string().when("agentType", {
    is: "Individual Registration",
    then: (schema) =>
      schema
        .required("Email is required")
  ...
  referAddress: yup.string().when("agentType", {
    is: "Individual Registration",
    then: (schema) =>
      schema
        .required("Address is required")
