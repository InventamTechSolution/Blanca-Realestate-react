...
export default function AboutPage() {
  ...
  // ✅ Loader (works fine in Next)
  useEffect(() => {
    const t = window.setTimeout(() => setIsLoading(false), 800);
    return () => window.clearTimeout(t);
  }, []);

  // ❌ REMOVE (Next handles scroll automatically)
  // useEffect(() => {
  //   window.scrollTo({ top: 0, left: 0, behavior: "instant" });
  // }, []);

  // ✅ Memo stays same
  const { teamItems, showcaseSlides, heroMessages } = useMemo(() => {
    ...
    return {
      teamItems: team,
      showcaseSlides: showcase,
      heroMessages: aboutMessages,
    };
  }, [aboutPageResponse]);
